...
function dimox_breadcrumbs() {
  ...
  $text['tag'] = '' . __( 'Записи с тегом', 'gpress' ) . ' "%s"'; // текст для страницы тега
  $text['author'] = '' . __( 'Статьи автора', 'gpress' ) . ' %s'; // текст для страницы автора
  $text['404'] = '' . __( 'Ошибка 404', 'gpress' ) . ''; // текст для страницы 404
  ...
  $wrap_before = '<div class="breadcrumbs gp-clearfix" itemscope itemtype="http://schema.org/BreadcrumbList">'; // открывающий тег обертки
  $wrap_after = '</div><!-- end breadcrumbs -->'; // закрывающий тег обертки
 ...
 if ( $show_on_home )  
 echo $wrap_before   . $home_link . $sep . sprintf( $text['page'], get_query_var( 'paged' ) ) . $wrap_after; // add bc to homepage pagination, by Goodwin
  } else {
